...
public class DiscordBot {
    ...
    private static final Logger LOGGER =
            Logger.fromClass(DiscordBot.class, TigerReportsSupports.getInstance().getName());
        ...
        if (token == null || token.isEmpty()) {
            Logger.CONFIG.error(
                    ConfigUtils.getInfoMessage(
                            "The Discord Bot token is not configured.",
        ...
        Bukkit.getScheduler().runTaskAsynchronously(trs, () -> {
            try {
                bot = JDABuilder.createDefault(token)
                        .addEventListeners(new DiscordListener(DiscordBot.this))
                        .build();
                bot.awaitReady();
                ...
                LOGGER.error(
                        ConfigUtils.getInfoMessage(
                                ...
                                "Une erreur est survenue avec Discord:"
                        ),
                        ex
                );
                return;
    ...
    public void notifyReport(String server, Report r) {
        ...
        String reporterName =
                r.getPlayerName(Report.ParticipantType.REPORTER, false, false, vm, null);
        String reportedName =
                r.getPlayerName(Report.ParticipantType.REPORTED, false, false, vm, null);
        
        String thumbnail =
                ConfigFile.CONFIG.get().getString("Config.Discord.Thumbnail", DEFAULT_THUMBNAIL);
        ...
        if (!DEFAULT_THUMBNAIL.equals(thumbnail)) {
            ...
            thumbnail = thumbnail.replace("_Reporter_", reporterName)
                    .replace("_Reported_", reportedName);
        }
        ...
        alert.setAuthor(
                messages.getString(path + "Title").replace("_Id_", Integer.toString(r.getId())),
                null,
        ...
        if (serverInfo) {
            alert.addField(
                    messages.getString(path + "Server"),
                    MessageUtils.getServerName(server),
                    true
            );
        }
    ...
    public void updateReportStatus(Report r) {
        try {
            ...
            c.getHistory()
                    .retrievePast(100)
                    .submit()
                    .whenComplete((List<Message> retrievedMessages, Throwable ex) -> {
                        if (ex == null) {
                                    ...
                                    fields.set(
                                            ...
                                            new Field(
                                                    ConfigFile.MESSAGES.get()
                                                            .getString(
                                                                    "DiscordMessages.Alert.Status"
                                                            ),
                                                    statusField,
                                                    false
                                            )
                                    );
                                    updatedAlert.setAuthor(
                                            alert.getAuthor().getName(),
                                            null,
                                            status.getIcon()
                                    );
